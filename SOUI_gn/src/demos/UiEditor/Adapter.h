...
public:
	...
	virtual SIZE getViewDesiredSize(int position, SOUI::SWindow *pItem, LPCRECT prcContainer)
	{
		...
		return SOUI::CSize(0, m_nItemHeight[viewType]);//cx在listview，mclistview中没有使用，不需要计算
	}
	...
	virtual void getView(int position, SOUI::SWindow * pItem, pugi::xml_node xmlTemplate)
	{
		if (pItem->GetChildrenCount() == 0)
		{
			...
			else
			{
				...
				pItem->InitFromXml(xmlTemplate.child(m_TemplateNames[(unsigned int)nViewType<m_TemplateNames.GetCount()-1?nViewType: m_TemplateNames.GetCount() - 1]));
			}
		}
	}	
};
