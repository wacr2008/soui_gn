...
private:
    SIZE            m_sizeMax;
	SOUI::SStringW        m_strImagePath;
	SOUI::SImagePlayer *  m_pPlayer;
    BOOL            m_bClosed;
};
...
protected:
	bool    OnFetchMore(SOUI::EventArgs *pEvt);
    LRESULT ProcessMessage(UINT msg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
    void    UpdatePosition();
};
...
public:
    struct NewsItem
    {
		SOUI::SStringW strTitle;
		SOUI::SStringW strDesc;
		SOUI::SStringW strURL;
		SOUI::SStringW strImagePath;
    };
    RichEditNewsOle();
    ~RichEditNewsOle();
...
protected:
	...
	bool        OnClickNotifyWnd(SOUI::EventArgs *pEvt);
    LRESULT     ProcessMessage(UINT msg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
    void        UpdatePosition();

private:
	typedef SOUI::SArray<NewsItem> ArrayNewsItem;
