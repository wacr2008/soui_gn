...
NSISAPI  InitWindow(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
	...
	extra->RegisterPluginCallback(g_hInstance, PluginCallback);
	{
		...
		SetCurrentDirectory(t.c_str());
		{
			...
			SASSERT(bLoaded);
#endif
			theApp->AddResProvider(pResProvider, _T("uidef:xml_init"));

			//加载系统资源
			wstring wTmpSYSUIStr = _T("");
			wTmpSYSUIStr += tmpPath;
			wTmpSYSUIStr += SYS_NAMED_RESOURCE;
			HMODULE hSysResource = LoadLibrary(wTmpSYSUIStr.c_str());
			if (hSysResource)
			{
			...
			{
				SOUI::SetMsgTemplate(xmlDoc.child(L"SOUI"));
			}
