...
namespace SOUI
{
	...
	SOUI_ATTRS_BEGIN(STabPageEx)
		ATTR_I18NSTRT(L"title", m_strTitle, FALSE)
		ATTR_INT(L"iconIndex", m_iIcon, FALSE)
	SOUI_ATTRS_END()

	CSize STabPageEx::GetDesiredSize(LPCRECT pRcContainer)
	{
		CSize size = __super::GetDesiredSize(pRcContainer);
		return CSize(size.cx, (std::max)((int)m_iHeight,(int) size.cy));
	}
	...
	bool STabCtrlEx::OnScrollviewOrginChanger(EventArgs *e)
	{
		...
		if (bUp) {
			...
			for (unsigned int i = 0; i < m_lstPages.GetCount(); i++)
			{
