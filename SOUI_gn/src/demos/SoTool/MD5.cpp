...
static void MD5Transform (UINT state[4], unsigned char block[64]);
static void Encode (unsigned char *output, UINT *input, unsigned int len);
static void Decode (UINT *output, unsigned char *input, unsigned int len);
...
context.
*/
void MD5Update (MD5_CTX *context, unsigned char *input, unsigned int inputLen,ICalcMd5ProgHandler *progHandler)
{
    ...
    */
    if (inputLen >= partLen) {
        ...
        for (i = partLen; i + 63 < inputLen; i += 64)
        {
            ...
            if(progHandler)
            {
                progHandler->OnCalcMd5Prog(inputLen,i);
            }
        }
...
/* Encodes input (UINT) into output (unsigned char). Assumes len is
a multiple of 4.
*/
static void Encode (unsigned char *output, UINT *input, unsigned int len)
{
