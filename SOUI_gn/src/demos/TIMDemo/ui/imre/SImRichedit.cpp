...
#define RETID_MAGELLANTRACK	0x01b3
	...
	SOUI_MSG_MAP_BEGIN(SImRichEdit)
...
#define _tomAllowOffClient  512  // Allow points outside of the client area.
    ...
    RichEditContent * SImRichEdit::GetContent(LPCWSTR pszName)
    {
        if (!pszName)
        {
            return NULL;
    ...
    BOOL SImRichEdit::GetContentIndex(LPCWSTR pszName, UINT& index)
    {
        for (size_t npos = 0; npos < _richContents.GetCount(); ++npos)
        {
            RichEditContent * p = _richContents.GetAt(npos);
    ...
    void SImRichEdit::ScrollTo(LPCWSTR pszName)
    {
    ...
    RichEditObj * SImRichEdit::GetElementById(INT64 nId)
    {
        for (size_t npos = 0; npos < _richContents.GetCount(); ++npos)
        {
            RichEditObj * pObjRet = _richContents.GetAt(npos)->GetById(nId);
            if (pObjRet != NULL)
                return pObjRet;
        }
    ...
    RichEditOleBase * SImRichEdit::GetOleById(INT64 nId)
    {
        RichEditObj* pObj = GetElementById(nId);
	...
	RichEditObj * SImRichEdit::GetElementByName(LPCWSTR lpszName)
	{
		for (size_t npos = 0; npos < _richContents.GetCount(); ++npos)
		{
			RichEditObj * pObjRet = _richContents.GetAt(npos)->GetByName(lpszName);
			if (pObjRet != NULL)
				return pObjRet;
		}
	...
	RichEditOleBase* SImRichEdit::GetOleByName(LPCTSTR lpszName)
	{
		RichEditObj* pObj = GetElementByName(lpszName);
    ...
    {
        HIMC hIMC = NULL;
		hIMC = ImmGetContext(GetContainer()->GetHostHwnd());
        if (hIMC)
        {
            POINT pt;
            GetCaretPos(&pt);
...
#define DTOHIMETRIC(d, dpi) (LONG)MulDiv(d, HIMETRIC_PER_INCH, dpi)
    ...
    BOOL SImRichEdit::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
    {
        ...
        {
            EventRichEditScroll evt(this);
            evt.nWheelDelta = zDelta;
            evt.bScrollAtTop = TRUE;
