...
namespace SOUI
{
	...
	{
		SOUI_CLASS_NAME_DECL(EventQueryAccept, L"on_re_query_accept_data")
	public:
		EventQueryAccept(SWindow *pSender)
			: TplEventArgs<EventQueryAccept>(pSender)
		{
		}
		enum { EventID = EVT_RE_NOTIFY + 101};
		RichFormatConv* pConv;
	};

	//class RichEditObj;
	class EventRichEditObj : public TplEventArgs < EventRichEditObj >
	{
		SOUI_CLASS_NAME_DECL(EventRichEditObj, L"on_re_ole")
	public:
		EventRichEditObj(SWindow *pSender) 
			: TplEventArgs<EventRichEditObj>(pSender)
		{
		}
		enum { EventID = EVT_RE_NOTIFY + 102 };
		RichEditObj*	pRichObj;
		int					nSubEventId;
		WPARAM        wParam;
		LPARAM			lParam;
	};
	...
	{
		SOUI_CLASS_NAME_DECL(EventRichEditScroll, L"on_re_scroll")
	public:
		EventRichEditScroll(SWindow *pSender) 
			: TplEventArgs<EventRichEditScroll>(pSender)
		{
			nWheelDelta = 0;
			bScrollAtTop = FALSE;
			bScrollAtBottom = FALSE;
		}
		enum { EventID = EVT_RE_NOTIFY + 103 };
		int nWheelDelta;
		BOOL bScrollAtTop;
		BOOL bScrollAtBottom;
    ...
    class SImRichEdit : public SRichEdit, public IRichEditObjHost, public ITimelineHandler
    {
        ...
        SOUI_CLASS_NAME_DECL(SImRichEdit, L"imrichedit")
    ...
    public:
        ...
        // extent methods
        //
        RichEditObj* GetElementById(INT64 nId);
		RichEditObj* GetElementByName(LPCWSTR lpszName);
        RichEditObj* HitTest(POINT ptInControl);
        RichEditContent* GetContent(LPCWSTR pszName);
        RichEditContent* GetContent(UINT uIndex);
        RichEditOleBase* GetOleById(INT64 nId);
		RichEditOleBase* GetOleByName(LPCTSTR lpszName);
        ...
        BOOL GetContentIndex(LPCWSTR pszName, UINT& index);
        void UpdateContentPosition(UINT start, UINT end = RECONTENT_LAST);
        void ScrollTo(LPCWSTR pszName);
