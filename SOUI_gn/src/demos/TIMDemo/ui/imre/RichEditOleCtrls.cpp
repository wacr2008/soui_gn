...
namespace SOUI
{
    ...
    ISkinObj* GetFileIconSkin(const SStringW& fileName)
    {
        ...
        ::SHGetFileInfo(fileName, FILE_ATTRIBUTE_NORMAL, &sfi, sizeof(SHFILEINFO), SHGFI_SYSICONINDEX|SHGFI_USEFILEATTRIBUTES);
	...
	SOUI_ATTRS_BEGIN(RichEditImageOle)
	...
	SOUI_ATTRS_BEGIN(RichEditFileOle)
	...
	SOUI_ATTRS_BEGIN(RichEditReminderOle)
	...
	SOUI_ATTRS_BEGIN(RichEditMetaFileOle)
    ...
    SStringW RichEditImageOle::MakeFormattedText(INT64 lId,
        ...
        BOOL showManifier)
    {
        ...
        formatedText.Format(L"<img id=\"%I64d\" subid=\"%s\" type=\"%s\" skin=\"%s\" path=\"%s\" encoding=\"%s\" show-magnifier=\"%d\"/>",
            lId,
            subId,
            type,
            skinId,
    ...
    HRESULT RichEditImageOle::InternalDraw(IRenderTarget* pRt, IRegion * prgn, DWORD cp)
    {
        ...
        if ((DWORD)chr.cpMin <= cp && cp < (DWORD)chr.cpMax && chr.cpMax - chr.cpMin > 1)
        {
		...
		LPCTSTR lpAttr)
    {
		...
		if(NULL == lpAttr)
			...
			formattedText.Format(L"<file %s selectable=\"0\" file-path=\"%s\" file-size=\"%I64d\" file-state=\"%s\" links=\"%d\" />",
			lpAttr,
			filePath,       // file-path
			fileSize,       // file-size
			fileState,      // file-state
    ...
    BOOL RichEditFileOle::InitOleWindow(IRichEditObjHost* pHost)
    {
        ...
        for (int i = 0; i < sizeof(links) / sizeof(links[0]); ++i)
        {
            ...
            if (pWnd)
            {
				pWnd->GetEventSet()->subscribeEvent(EVT_CMD, Subscriber(&RichEditFileOle::OnLinkClicked,this));
            }
        }
    ...
    void RichEditFileOle::SetFilePath(const SStringW& path)
    {
        ...
        if (pImageWin)
        {
			pImageWin->SetIcon(GetFileIconIndex(_fileName));
    ...
    void RichEditFileOle::SetFileLinksVisible(int links)
    {
        SWindow * pWnd = NULL;
        BOOL visible = FALSE;
		pWnd = _oleView.FindChildByName(L"LnkSaveFile");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkSaveFileAs");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkCancelFile");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkOpenFile");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkOpenFileDir");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkContinueFile");
        if (pWnd)
        {
		...
		pWnd = _oleView.FindChildByName(L"LnkForwardFile");
        if (pWnd)
        {
    ...
    // {C0402A65-5BBF-4bef-9861-C55CD3A07201}
     const GUID IID_FetchMoreOleCtrl =
    ...
    BOOL RichEditFetchMoreOle::InitOleWindow(IRichEditObjHost * pHost)
    {
        ...
        if (pLink)
        {
			pLink->GetEventSet()->subscribeEvent(EVT_CMD, Subscriber(&RichEditFetchMoreOle::OnClickFetchMore, this));
        }

        pLink = static_cast<SLink*>(_oleView.FindChildByName(L"LnkOpenHistory"));
        if (pLink)
        {
			pLink->GetEventSet()->subscribeEvent(EVT_CMD, Subscriber(&RichEditFetchMoreOle::OnClickOpenHistory, this));
		}
    ...
    // {8D5E6EF1-2A36-4930-91E7-1149303994BF}
     const GUID IID_SeparatorBarCtrl =
    ...
    // {A5D897A6-4533-4e15-9A33-A5814F927768}
     const GUID IID_RemainderOleCtrl =
    ...
    // {3AAFAC01-0C3D-447a-AEDB-8C88FA1CF7B5}
     const GUID IID_RichMetaFileOle =
