...
void GSTabCtrl::OnPaint(IRenderTarget *pRT)
{
	...
	for (int i = 0; i<GetItemCount(); i++)
	{
		...
		if (i == (int)m_nCurrentPage) dwState = WndState_PushDown;
		else if (i == (int)m_nHoverTabItem) dwState = WndState_Hover;
...
void GSTabCtrl::OnNextFrame()
{
	...
	for (unsigned int i = 0; i < m_arrHoverNumber.GetCount(); i++)
	{
