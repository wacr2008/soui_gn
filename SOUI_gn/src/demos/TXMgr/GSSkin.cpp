...
SOUI_ATTRS_BEGIN(GSSkinImgList)
...
void GSSkinImgList::_Draw(IRenderTarget *pRT, LPCRECT rcDraw, DWORD dwState, BYTE byAlpha)
{
	...
	if (!m_bmpMask)
	{
		if (m_bTile && (pImg->Height() > (UINT)(rcDraw->bottom - rcDraw->top) || pImg->Width() > (UINT)(rcDraw->right - rcDraw->left)) && (fOffsetX != 0 || fOffsetY != 0))
		{
...
{
	for (unsigned int i = 0; i < m_arrBitMap.GetCount(); i++)
	{
	...
	for (unsigned int i = 0;i < fontProp.GetCount(); i++)
	{
