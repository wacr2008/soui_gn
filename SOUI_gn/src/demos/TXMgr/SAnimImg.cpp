...
namespace SOUI
{
	...
	SOUI_ATTRS_BEGIN(SAnimImg)
		...
		ATTR_INT(L"playall", m_iPlayAll, FALSE)
		ATTR_SKIN(L"ani_in", m_pAniIn, TRUE)
		ATTR_SKIN(L"ani_out", m_pAniOut, TRUE)
		ATTR_SKIN(L"static", m_pStatic, TRUE)
    ...
    CSize SAnimImg::GetDesiredSize(LPCRECT pRcContainer)
    {
		if (m_pAniIn)
		{
			...
			{
				szRet.cx = GetLayoutParam()->GetSpecifiedSize(Horz).unit;
			}
			if(GetLayoutParam()->IsSpecifiedSize(Vert))
			{
				szRet.cy = GetLayoutParam()->GetSpecifiedSize(Vert).unit;
			}
			return szRet;
		}
		return NULL;
