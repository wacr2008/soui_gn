...
namespace SOUI
{
	...
	SOUI_MSG_MAP_BEGIN(SShellNotifyIcon)
		MSG_WM_PAINT_EX(OnPaint)
		MSG_WM_ERASEBKGND_EX(OnEraseBkgnd)
		MSG_WM_NCPAINT_EX(OnPaint)
	SOUI_MSG_MAP_END()
	
	SOUI_ATTRS_BEGIN(SShellNotifyIcon)
	...
	{
		if ((unsigned int )iIdx >= m_ArrIcon.GetCount())
			return;
		m_iDefIcon = iIdx;
		if (!m_bRunAni)
		{
	...
	{
		if ((unsigned int )m_iCurFrame >= m_ArrIcon.GetCount())
			m_iCurFrame = m_iStartFrame;
	...
	BOOL SShellNotifyIcon::CreateChildren(pugi::xml_node xmlNode)
	{
		...
		SASSERT((unsigned int)m_iDefIcon < m_ArrIcon.GetCount());
		//SASSERT(m_ArrIcon.GetCount()>0);
		if ((unsigned int )m_iDefIcon >= m_ArrIcon.GetCount())
			m_iDefIcon = 0;
