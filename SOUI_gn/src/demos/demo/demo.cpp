...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPTSTR /*lpstrCmdLine*/, int /*nCmdShow*/)
{
    ...
    */
...

    {
        ...
        ZIPRES_PARAM param;
        param.ZipFile(pRenderFactory, _T("uires.zip"),"souidemo");
        bLoaded = pResProvider->Init((WPARAM)&param,0);
        SASSERT(bLoaded); 
#elif (RES_TYPE==3)//从7ZIP包加载
		bLoaded = pComMgr->CreateResProvider_ZIP((IObjRef**)&pResProvider);
		SASSERT_FMT(bLoaded, _T("load interface [%s] failed!"), _T("resprovider_zip"));

		std::wstring s7zFileName = strResDir + _T("\\uires.zip");
		ZIP7RES_PARAM param;
		param.ZipFile(pRenderFactory, s7zFileName.c_str(), ("souidemo"));
		bLoaded = pResProvider->Init((WPARAM)&param, 0);
		SASSERT(bLoaded); 
#endif 
        ...
        {//加载语言翻译包
            ...
            if(theApp->LoadXmlDocment(xmlLang,_T("lang_cn"),_T("translator")))
            {
				...
				langCN->Load((LPVOID)&node, 1);//1=LD_XML
		...
		{
			::MessageBox(NULL,_T("提示"),_T("不行，系统资源dll不存在"),0);
		}
        //采用hook绘制菜单的边框
