...
public:
	...
	{
		for(unsigned int i=0;i<m_lstTagCheck.GetCount();i++)
		{
...
public:
	...
	void SetPids(const SArray<UINT> & pids)
	{
		...
		for(unsigned int i=0;i<pids.GetCount();i++)
		{
	...
	void MarkAll(bool bSelected)
	{
		for(unsigned int i=0;i<m_lstPidCheck.GetCount();i++)
		{
...
public:
	...
	void SetTids(const SArray<UINT> & tids)
	{
		...
		for(unsigned int i=0;i<tids.GetCount();i++)
		{
	...
	void MarkAll(bool bSelected)
	{
		for(unsigned int i=0;i<m_lstTidCheck.GetCount();i++)
		{
