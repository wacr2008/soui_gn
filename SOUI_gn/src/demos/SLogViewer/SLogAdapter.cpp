...
namespace SOUI
{
	...
	void SLogBuffer::Insert(const SLogBuffer & src)
	{
		...
		for(unsigned int i=0;i<src.m_lstLogs.GetCount();i++)
		{
			m_lstLogs[i]->AddRef();
		}
		for(unsigned int i=src.m_lstLogs.GetCount();i<m_lstLogs.GetCount();i++)
		{
	...
	{
		for(unsigned int i=0;i<m_lstLogs.GetCount();i++)
		{
			m_lstLogs[i]->Release();
		}
		m_lstLogs.RemoveAll();
	...
	void SLogAdapter::getView(int position, SWindow * pItem,pugi::xml_node xmlTemplate)
	{
		...
		for(unsigned int i=0;i<hilightRange.GetCount();i++)
		{
...
// 	}
	...
	SLogInfo* SLogAdapter::GetLogInfo(int iItem) const
	{
		...
		if(pLogInfos->GetCount()<=(unsigned int)iItem) return NULL;
	...
	{
		m_filterTags.RemoveAll();
		for(unsigned int i=0;i<tags.GetCount();i++)
		{
	...
	{
		m_filterPids.RemoveAll();
		for(unsigned int i=0;i<lstPid.GetCount();i++)
		{
	...
	{
		m_filterTids.RemoveAll();
		for(unsigned int i=0;i<lstTid.GetCount();i++)
		{
	...
	{
		outRange.RemoveAll();
		for(unsigned int i=0;i<m_lstInclude.GetCount();i++)
		{
	...
	{
		for(unsigned int i=0;i<m_lstExclude.GetCount();i++)
		{
