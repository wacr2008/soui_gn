...
public:
	...
	//播放歌曲与滚动歌词 预览开始
	void playingStart(SOUI::SHostWnd *wnd);

	//预览结束
	void playingEnd(SOUI::SHostWnd *wnd);
...
{
public:
	TimeLineInfo(SOUI::SStringT timeLine)
	{
		//初始化类的基本成员的信息
		m_strTimeLine = timeLine;
		int pos = m_strTimeLine.Find(_T(']'));
		SOUI::SStringT timeStr = m_strTimeLine.Left(pos + 1);
...
public:
	SOUI::SStringT m_strTimeLine;		/* 直接存储 从文件读取的一整行 */
	SOUI::SStringT m_strLine;			/* 存储去除时间标记之后的内容 */
