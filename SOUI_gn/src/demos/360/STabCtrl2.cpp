...
namespace SOUI
{
	...
	SOUI_MSG_MAP_BEGIN(STabCtrl2)
		MSG_WM_CREATE(OnCreate)
		MSG_WM_DESTROY(OnDestroy)
		MSG_WM_PAINT_EX(OnPaint)
	SOUI_MSG_MAP_END()

	SOUI_ATTRS_BEGIN(STabCtrl2)
	...
	void STabCtrl2::OnPaint(IRenderTarget *pRT)
	{
		...
		for(int i=0; i<GetItemCount(); i++)
		{
			...
			if((int)i == m_nCurrentPage) dwState=WndState_PushDown;
			else if ((int)i == m_nHoverTabItem) dwState = WndState_Hover;

			GetItemRect(i,rcItem);
			//画分隔线
			if (i>0 && m_pSkinTabInter && (((int)i < (m_nCurrentPage)) || ((int)i >(m_nCurrentPage + 1))/*选中页左右不用画分隔线*/))
			{ 
				rcSplit=rcItem;
