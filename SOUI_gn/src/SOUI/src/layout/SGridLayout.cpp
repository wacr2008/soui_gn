...
namespace SOUI
{
	SOUI_ATTRS_BEGIN(SGridLayoutParam)
	...
	GridGravity SGridLayoutParam::parseGridGravity(const SStringW & strValue)
	{
		...
		for(unsigned int i=0;i<ARRAYSIZE(map);i++)
		{
	...
	SGridLayout::SGridLayout(void):m_nCols(-1),   m_nRows(-1), m_GravityX(gCenter), m_GravityY(gCenter)
	{
	}
