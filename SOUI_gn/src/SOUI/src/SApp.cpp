...
{
public:
    BOOL CreateTranslator(ITranslator **pLang) override {return FALSE;}
    BOOL InstallTranslator(ITranslator * pLang) override {return FALSE;}
    BOOL UninstallTranslator(REFGUID id) override {return FALSE;}
    SStringW tr(const SStringW & strSrc,const SStringW & strCtx) override
    {
        return strSrc;
    } 


	void SetLanguage(const SStringW & strLang) override
	{
	}

	SStringW GetLanguage() const override
	{
		return SStringW();
	}
...
{
public:
    /*virtual */IToolTip * CreateToolTip(HWND hHost) override
    {
    ...
    /*virtual */void DestroyToolTip(IToolTip *pToolTip) override
    {
        if(pToolTip)
        {
...
{
public:
	~SDefMsgLoopFactory() override
	{

	}

    SMessageLoop * CreateMsgLoop() override
    {
        return new SMessageLoop;
    }

    void DestoryMsgLoop(SMessageLoop * pMsgLoop) override
    {
        delete pMsgLoop;
    }
};
...
BOOL SApplication::_LoadXmlDocment( LPCTSTR pszXmlName ,LPCTSTR pszType ,pugi::xml_document & xmlDoc,IResProvider *pResProvider/* = NULL*/)
{
    if(!pResProvider) 
    {
        if(IsFileType(pszType))
        {
			...
			SStringT desc = S_CA2W(result.description());
            SASSERT_FMTW(false,L"parse xml error! xmlName=%s,desc=%s,offset=%d",(const wchar_t *)pszXmlName, (const wchar_t *)desc,(int)result.offset);
            return result;
        }else
        {
	...
	SStringT desc = S_CA2W(result.description());
    SASSERT_FMTW(true,L"parse xml error! xmlName=%s,desc=%s,offset=%d", (const wchar_t *)pszXmlName,(const wchar_t *)desc,(int)result.offset);
    return result;
}
