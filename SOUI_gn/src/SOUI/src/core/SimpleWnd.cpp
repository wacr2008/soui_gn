...
namespace SOUI
{
    ...
    CSimpleWndHelper::~CSimpleWndHelper()
    {
		...
		DWORD name=0;
		name= m_atom&0x0000FFFF;
        if(m_atom) UnregisterClass((LPCTSTR)name,m_hInst);
    }
...
BOOL CSimpleWnd::ProcessWindowMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lResult, DWORD dwMsgMapID ) 
{
	return FALSE;
}
...
HWND CSimpleWnd::Create(LPCTSTR lpWindowName, DWORD dwStyle,DWORD dwExStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent,LPVOID lpParam )
{
	...
	DWORD name = 0;
	name = CSimpleWndHelper::GetInstance()->GetSimpleWndAtom() & 0x0000FFFF;

    HWND hWnd= ::CreateWindowEx(dwExStyle,(LPCTSTR)name, lpWindowName, dwStyle, x, y, nWidth, nHeight, hWndParent, 0, CSimpleWndHelper::GetInstance()->GetAppInstance(), lpParam);
