...
//    SDummyWnd
//////////////////////////////////////////////////////////////////////////
	SDummyWnd::SDummyWnd(SHostWnd* pOwner) :m_pOwner(pOwner)
	{
	}
...
BEGIN_MSG_MAP_EX(SHostWnd)
	...
	MSG_WM_SETCURSOR(OnWndSetCursor)
...
int SHostWnd::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
	...
	if(!m_strXmlLayout.IsEmpty())
	{
		...
		{
			SASSERT_FMTA(FALSE,"Load layout [%s] Failed",(const char *)S_CT2A(m_strXmlLayout));
		}
	}
    return 0;
}
...
BOOL SHostWnd::OnWndSetCursor(HWND hwnd, UINT nHitTest, UINT message)
{
...
BOOL SHostWnd::IsLayeredWindow() const 
{
	return FALSE; 
}

BOOL SHostWnd::_HandleEvent(EventArgs *pEvt) 
{
	return FALSE; 
}
