...
namespace SOUI
{
	SPainter::SPainter() : oldTextColor(CR_INVALID)
	{
	} 
	...
	SwndToolTipInfo::SwndToolTipInfo()
	{
	...
	SMsgHandleState::SMsgHandleState() :m_bMsgHandled(FALSE)
	{
	...
	void SWindow::_ReleaseRenderTarget(IRenderTarget *pRT)
	{
		...
		{//存在一个渲染层
			...
			if(m_pGetRTData->gdcFlags != OLEDC_NODRAW)
			{
				...
				//绘制当前layer前的窗口前景
				if(uFrgndZorderMin!=(UINT)ZORDER_MAX)
	...
	HRESULT SWindow::OnAttrID( const SStringW& strValue, BOOL bLoading )
	{
		...
		if(!strValue.IsEmpty())
		{
			...
			{
				for(unsigned int i=0;i<ARRAYSIZE(systemID);i++)
				{
	...
	void SWindow::SetName(LPCWSTR pszName) 
	{
		m_strName = pszName; 
	}

	int SWindow::GetID() const 
	{
		return m_nID; 
	}

	void SWindow::SetID(int nID) 
	{
		m_nID = nID; 
	}
	...
	SWindow *SWindow::GetSelectedSiblingInGroup()
	{
		return NULL; 
	}
