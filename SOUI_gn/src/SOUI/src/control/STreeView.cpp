...
namespace SOUI 
{
	...
	public:
		...
		void onBranchChanged(HTREEITEM hBranch) override
		{
		    m_pOwner->onBranchChanged(hBranch);
		}
		
		void onBranchInvalidated(HTREEITEM hBranch) override
		{
		    m_pOwner->onBranchInvalidated(hBranch);
		}
		
        void onBranchExpandedChanged(HTREEITEM hBranch,BOOL bExpandedOld,BOOL bExpandedNew) override
        {
	...
	LRESULT STreeView::OnMouseEvent( UINT uMsg,WPARAM wParam,LPARAM lParam )
	{
		...
		if(m_itemCapture) {
			...
			{//交给panel处理
				CPoint pt_tmp = pt;
				SItemPanel* pPanel = HitTest(pt_tmp);
