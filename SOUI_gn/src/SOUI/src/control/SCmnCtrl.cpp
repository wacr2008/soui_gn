...
UINT SButton::OnGetDlgCode()
{
	return SC_WANTCHARS;
}

BOOL SButton::NeedRedrawWhenStateChange()
{
	return TRUE;
}
...
SOUI_MSG_MAP_BEGIN(SAnimateImgWnd)
	MSG_WM_PAINT_EX(OnPaint)
	MSG_WM_DESTROY(OnDestroy)
...
BOOL SCheckBox::NeedRedrawWhenStateChange()
{
	return TRUE;
}

UINT SCheckBox::OnGetDlgCode()
{
	return SC_WANTCHARS;
}
...
SOUI_ATTRS_BEGIN(SIconWnd)
	ATTR_ICON(L"src", m_theIcon, FALSE)
SOUI_ATTRS_END()


SOUI_MSG_MAP_BEGIN(SIconWnd)
	MSG_WM_PAINT_EX(OnPaint)
SOUI_MSG_MAP_END()
...
UINT SRadioBox::OnGetDlgCode()
{
	return 0;
}

BOOL SRadioBox::IsSiblingsAutoGroupped() 
{
	return TRUE; 
}
...
SOUI_ATTRS_BEGIN(SToggle)
	ATTR_INT(L"toggled", m_bToggled, TRUE)
	ATTR_SKIN(L"skin", m_pSkin, TRUE)
SOUI_ATTRS_END()

SOUI_MSG_MAP_BEGIN(SToggle)
	MSG_WM_PAINT_EX(OnPaint)
	MSG_WM_LBUTTONUP(OnLButtonUp)
SOUI_MSG_MAP_END()
...
BOOL SToggle::NeedRedrawWhenStateChange() 
{
	return TRUE; 
}
