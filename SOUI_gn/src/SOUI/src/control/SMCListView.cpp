...
void SMCListView::UpdateScrollBar()
{
    ...
    //  根据需要调整原点位置
    if (HasScrollBar(FALSE) && m_siHoz.nPos+m_siHoz.nPage>(UINT)szView.cx)
    {
        m_siHoz.nPos = szView.cx-m_siHoz.nPage;
    }

    if (HasScrollBar(TRUE) && m_siVer.nPos +m_siVer.nPage>(UINT)szView.cy)
    {
...
bool SMCListView::OnHeaderClick(EventArgs *pEvt)
{
    ...
    int iCol = -1;
    for(unsigned int i=0;i<m_pHeader->GetItemCount();i++)
    {
        m_pHeader->GetItem(i,&hi);
        pstFlags[hi.iOrder]=hi.stFlag;
        pOrders[hi.iOrder]=i;
        if(i == (unsigned int)pEvt2->iItem) iCol = hi.iOrder;
    }
