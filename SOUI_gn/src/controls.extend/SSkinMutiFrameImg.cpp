...
namespace SOUI
{
	...
	SOUI_ATTRS_BEGIN(SSkinMutiFrameImg)
		ATTR_CUSTOM(L"src", OnAttrSrc)   //XML文件中指定的图片资源名,(type:name)
	SOUI_ATTRS_END()


    LRESULT SSkinMutiFrameImg::OnAttrSrc( const SStringW &strValue,BOOL bLoading )
    {
        SStringTList strLst;
...
int SSkinMutiFrameImg::LoadFromFile( LPCTSTR pszFileName )
{
...
int SSkinMutiFrameImg::LoadFromMemory( LPVOID pBuf,size_t dwSize )
{
...
int SSkinMutiFrameImg::LoadFromGdipImage( Gdiplus::Bitmap * pImage )
{
...
BOOL SSkinMutiFrameImg::Gdiplus_Startup()
{
...
void SSkinMutiFrameImg::Gdiplus_Shutdown()
{
