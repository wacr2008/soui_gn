...
typedef std::vector<SOUI::SStringW> StringList;

class EventItemSelected : public SOUI::TplEventArgs<EventItemSelected>
{
...
// 回车就要选中
// 上下移动不要关闭
class DropDownListWnd : public SOUI::SDropDownWnd
{
public:
	DropDownListWnd(SOUI::ISDropDownOwner* pOwner) :SDropDownWnd(pOwner),
        m_pListBox(NULL)
    {
    ...
    BOOL PreTranslateMessage( MSG* pMsg )
    {
        ...
        if (pMsg->message == WM_MOUSEMOVE && m_pListBox)
        {
			...
			SOUI::CRect rc = m_pListBox->GetClientRect();
            if (rc.PtInRect(pt))
            {
    ...
    virtual BOOL Create(LPCRECT lpRect,LPVOID lParam,DWORD dwStyle=WS_POPUP,DWORD dwExStyle=WS_EX_TOOLWINDOW|WS_EX_TOPMOST)
    {
        ...
        if(xmlDoc)
        {
			...
			m_pListBox = FindChildByName2<SOUI::SListBoxEx>(L"selectionlist");
	...
	BOOL FireEvent(SOUI::EventArgs &evt)
    {
		if (evt.GetID() == SOUI::EventOfPanel::EventID)
        {
			SOUI::EventOfPanel *pEvtOfPanel = (SOUI::EventOfPanel*)&evt;
			if (pEvtOfPanel->pOrgEvt->GetID() == SOUI::EventCmd::EventID)
            {
                EventItemSelected evt(this);
                evt.nCurrSelect = m_pListBox->GetCurSel();
...
private:
	SOUI::SListBoxEx * m_pListBox;
};


class GroupChatFrame : public SOUI::SHostWnd
	, public SOUI::ISDropDownOwner
{
public:
    GroupChatFrame();
    ~GroupChatFrame();
...
protected:
	...
	virtual void OnCreateDropDown(SOUI::SDropDownWnd *pDropDown);
	virtual void OnDestroyDropDown(SOUI::SDropDownWnd *pDropDown);

	BOOL GetEditorCursorRect(SImRichEdit * pRichEdit, LONG cpStart, LONG cpEnd, SOUI::CRect &rc);
	void ShowMemberSelectionWnd(const SOUI::CRect& rcCursor, const SOUI::SStringW& strKeyWord);
